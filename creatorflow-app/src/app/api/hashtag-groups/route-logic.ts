export async function handleGetHashtagGroups({ req, getSession, prisma }) {
...
export async function handleCreateHashtagGroup({ req, getSession, prisma }) {
  const session = await getSession(req);
  if (!session?.user?.id) return { status: 401, body: { error: 'Unauthorized' } };
...
export async function handleUpdateHashtagGroup({ req, getSession, prisma }) {
  const session = await getSession(req);
  if (!session?.user?.id) return { status: 401, body: { error: 'Unauthorized' } };
...
export async function handleDeleteHashtagGroup({ req, getSession, prisma }) {
  const session = await getSession(req);
  if (!session?.user?.id) return { status: 401, body: { error: 'Unauthorized' } };
...
export async function handlePatchHashtagGroup({ req, getSession, prisma }) {
  const session = await getSession(req);
  if (!session?.user?.id) return { status: 401, body: { error: 'Unauthorized' } };
