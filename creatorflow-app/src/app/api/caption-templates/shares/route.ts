...
// GET: List shares for a template or for the user
export async function GET(req: NextRequest) {
  ...
  let where: any = {};
  if (templateId && type) {
    where = { [`${type}TemplateId`]: templateId };
  } else {
...
export async function POST(req: NextRequest) {
  ...
  const { templateId, type, sharedWithEmail, permissions } = await req.json();
  if (!templateId || !type || !sharedWithEmail || !permissions) return NextResponse.json({ error: 'Missing fields' }, { status: 400 });
  const data: any = { userId, sharedWithEmail, permissions, type };
  if (type === 'caption') data.captionTemplateId = templateId;
  else if (type === 'hashtag') data.hashtagGroupId = templateId;
